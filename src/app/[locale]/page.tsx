...
import { ArrowRight, ChevronRight, Shield, Swords, DollarSign, PawPrint, Sparkles, Crown, Flame, Home, Zap, Gift, BookOpen, Dice6, Wrench, Brain, HelpCircle, Megaphone, Users, Gamepad2, TrendingUp, Lock, Target } from 'lucide-react';
...
export default async function HomePage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const dynCtx: Record<string, string> = {
    ...
    '__petCount': String(pets.length),
    '__legendaryCount': String(PET_STATS.legendaryCount),
    '__gearCount': String(gear.length),
    '__gearCategoryCount': String(4), // weapon, shield, tool, boost
    '__activeCodesCount': '0',
    '__totalCodesCount': '0',
    '__rarityCount': '4',
    '__defenseStrategyCount': '4',
    '__baseDesignCount': String(BASE_DESIGN_STATS.totalCount),
    '__strategyCount': '5',
    '__proTipCount': '8',
    '__offlineStrategyCount': '6',
    '__maxOfflineHours': '12',
    '__faqCount': '10',
    '__commonIssueCount': '8',
    '__updateCount': '3',
  };

  const heroBadgeKeys = HOME_CONFIG.hero.badgeKeys;
  const moduleCards = HOME_CONFIG.modules.map((card) => ({
    ...card,
  ...
  const startHereSteps = HOME_CONFIG.startHere.steps.map((step) => ({
    number: step.number,
    ...
    <div>
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-5 mb-10">
          {[
            { icon: PawPrint, title: 'pet_collecting', desc: 'pet_collecting_desc' },
            { icon: Shield, title: 'base_building', desc: 'base_building_desc' },
            { icon: Swords, title: 'raiding_system', desc: 'raiding_system_desc' },
            { icon: DollarSign, title: 'passive_income', desc: 'passive_income_desc' },
          ].map((feat, idx) => {
            ...
            return (
              <div key={idx} className="rounded-2xl bg-white/[0.03] border border-white/[0.08] p-5 backdrop-blur-sm hover:border-[var(--color-accent)]/30 transition-all duration-300">
                ...
                <h3 className="text-sm font-bold mb-2">{t(`home_feature_${feat.title}`)}</h3>
                <p className="text-xs text-[var(--color-text-secondary)] leading-relaxed">{t(`home_feature_${feat.desc}`)}</p>
              </div>
            );
          })}
        </div>
        ...
        <div className="grid lg:grid-cols-2 gap-8">
          ...
          <div className="grid grid-cols-2 gap-3">
            {['developer', 'platform', 'genre', 'visits', 'favorites', 'server', 'updated'].map((item) => (
      ...
      {/* ===== PETS TABLE (Data-Driven Table 1) ===== */}
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        <div className="flex items-center justify-between mb-8">
          <div>
            <div className="section-label">{t('home_module_petsList')}</div>
            <h2 className="text-2xl md:text-3xl font-bold font-[var(--font-heading)] gradient-text">{t('home_module_pets_title')}</h2>
          </div>
          <Link href="/pets-list" className="text-sm font-semibold text-[var(--color-accent)] hover:underline flex items-center gap-1">{t('view_all')} <ArrowRight className="w-3 h-3" /></Link>
        </div>
        <p className="text-[var(--color-text-secondary)] mb-6 leading-relaxed">{t('home_module_pets_desc')}</p>
        <div className="rounded-2xl bg-white/[0.02] border border-white/[0.08] overflow-hidden backdrop-blur-sm">
          <div className="grid grid-cols-[2rem_1fr_1fr_1fr_1fr] sm:grid-cols-[2.5rem_1fr_1fr_1fr_1fr] gap-4 text-xs font-semibold text-[var(--color-text-muted)] uppercase tracking-wider border-b border-[var(--color-border)] px-4 py-3">
            <span></span><span>Pet</span><span>Tier</span><span>Rarity</span><span>Type</span>
          </div>
          {pets.slice(0, 8).map((pet, idx) => {
            const tc = tierColor(pet.tier);
            const rc = rarityColor(pet.rarityKey.replace('pet_rarity_', ''));
            const Icon = PET_ICONS[pet.typeKey.replace('pet_type_', '')] || PawPrint;
            return (
              <Link key={pet.id} href="/pets-list" className="grid grid-cols-[2rem_1fr_1fr_1fr_1fr] sm:grid-cols-[2.5rem_1fr_1fr_1fr_1fr] items-center gap-4 px-4 py-3 border-b border-white/[0.04] last:border-b-0 hover:bg-[var(--color-accent)]/5 transition-colors group">
                ...
                <span className="flex items-center gap-2">
                  ...
                  <span className="font-semibold text-sm group-hover:text-[var(--color-accent)] transition-colors">{translate(t, pet.nameKey) || pet.id}</span>
                </span>
                <span><span className="text-[10px] font-bold px-2 py-0.5 rounded-full" style={{ color: tc, background: `${tc}15` }}>{pet.tier}</span></span>
                <span><span className="text-[10px] font-bold px-2 py-0.5 rounded-full" style={{ color: rc, background: `${rc}15` }}>{translate(t, pet.rarityKey)}</span></span>
                <span className="text-xs text-[var(--color-text-secondary)]">{translate(t, pet.typeKey)}</span>
              </Link>
            );
          })}
        </div>
      ...
      {/* ===== RARITY CARDS (Data-Driven Cards 1) ===== */}
      <section className="bg-[var(--color-bg-secondary)] border-y border-[var(--color-border)] full-width-bg">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
          <div className="flex items-center justify-between mb-8">
            <div>
              <div className="section-label">{t('home_module_rarities')}</div>
              <h2 className="text-2xl md:text-3xl font-bold font-[var(--font-heading)] gradient-text">{t('home_module_rarities_title')}</h2>
            </div>
            <Link href="/pets-list" className="text-sm font-semibold text-[var(--color-accent)] hover:underline flex items-center gap-1">{t('view_all')} <ArrowRight className="w-3 h-3" /></Link>
          </div>
          <p className="text-[var(--color-text-secondary)] mb-6 leading-relaxed">{t('home_module_rarities_desc')}</p>
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-5">
            {rarityCards.map((rarity) => {
              const rc = rarityColor(rarity.id);
              const Icon = RARITY_ICONS[rarity.id] || Sparkles;
              return (
                <Link key={rarity.id} href="/pets-list" className="block rounded-2xl bg-white/[0.03] border border-white/[0.08] p-5 backdrop-blur-sm hover:border-[var(--color-accent)]/30 transition-all duration-300 group">
                  <div className="flex items-center justify-between mb-3">
                    <span className="text-[10px] font-bold px-2 py-0.5 rounded-full" style={{ color: rc, background: `${rc}15` }}>{translate(t, rarity.nameKey)}</span>
                    <Icon className="w-4 h-4" style={{ color: rc }} />
                  </div>
                  <h3 className="font-bold mb-1 group-hover:text-[var(--color-accent)] transition-colors">
                    {translate(t, rarity.nameKey)}
                  </h3>
                  <p className="text-sm text-[var(--color-text-secondary)] mb-2">{translate(t, rarity.descriptionKey)}</p>
                  <span className="text-xs text-[var(--color-text-muted)]">{rarity.petCount} pets</span>
                </Link>
              );
            })}
          </div>
      ...
      {/* ===== GEAR TABLE (Data-Driven Table 2) ===== */}
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        <div className="flex items-center justify-between mb-8">
          <div>
            <div className="section-label">{t('home_module_gear')}</div>
            <h2 className="text-2xl md:text-3xl font-bold font-[var(--font-heading)] gradient-text">{t('home_module_gear_title')}</h2>
          </div>
          <Link href="/gear" className="text-sm font-semibold text-[var(--color-accent)] hover:underline flex items-center gap-1">{t('view_all')} <ArrowRight className="w-3 h-3" /></Link>
        </div>
        <p className="text-[var(--color-text-secondary)] mb-6 leading-relaxed">{t('home_module_gear_desc')}</p>
        <div className="rounded-2xl bg-white/[0.02] border border-white/[0.08] overflow-hidden backdrop-blur-sm">
          <div className="grid grid-cols-[2rem_1fr_1fr_1fr] sm:grid-cols-[2.5rem_1fr_1fr_1fr] gap-4 text-xs font-semibold text-[var(--color-text-muted)] uppercase tracking-wider border-b border-[var(--color-border)] px-4 py-3">
            <span></span><span>Gear</span><span>Type</span><span>Effect</span>
          </div>
          {gear.slice(0, 6).map((item, idx) => {
            const Icon = GEAR_ICONS[item.typeKey.replace('gear_type_', '')] || Wrench;
            return (
              <Link key={item.id} href="/gear" className="grid grid-cols-[2rem_1fr_1fr_1fr] sm:grid-cols-[2.5rem_1fr_1fr_1fr] items-center gap-4 px-4 py-3 border-b border-white/[0.04] last:border-b-0 hover:bg-[var(--color-accent)]/5 transition-colors group">
                <span className="text-xs font-mono text-[var(--color-text-muted)]">{idx + 1}</span>
                <span className="flex items-center gap-2">
                  <Icon className="w-4 h-4 text-[var(--color-accent)] shrink-0" />
                  <span className="font-semibold text-sm group-hover:text-[var(--color-accent)] transition-colors">{translate(t, item.nameKey) || item.id}</span>
                </span>
                <span className="text-xs text-[var(--color-text-secondary)]">{translate(t, item.typeKey)}</span>
                <span className="text-xs text-[var(--color-text-secondary)]">{translate(t, item.effectKey)}</span>
              </Link>
            );
          })}
        </div>
      ...
      {/* ===== BASE DESIGN CARDS (Data-Driven Cards 2) ===== */}
      <section className="bg-[var(--color-bg-secondary)] border-y border-[var(--color-border)] full-width-bg">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
          <div className="flex items-center justify-between mb-8">
            <div>
              <div className="section-label">{t('home_module_baseDefense')}</div>
              <h2 className="text-2xl md:text-3xl font-bold font-[var(--font-heading)] gradient-text">{t('home_module_baseDefense_title')}</h2>
            </div>
            <Link href="/base-defense" className="text-sm font-semibold text-[var(--color-accent)] hover:underline flex items-center gap-1">{t('view_all')} <ArrowRight className="w-3 h-3" /></Link>
          </div>
          <p className="text-[var(--color-text-secondary)] mb-6 leading-relaxed">{t('home_module_baseDefense_desc')}</p>
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-5">
            {baseDesignCards.map((design) => {
              const Icon = BASE_DESIGN_ICONS[design.id] || Shield;
              return (
                <Link key={design.id} href="/base-defense" className="block rounded-2xl bg-white/[0.03] border border-white/[0.08] p-5 backdrop-blur-sm hover:border-[var(--color-accent)]/30 transition-all duration-300 group">
                  <div className="flex items-center justify-between mb-3">
                    <span className="text-[10px] font-bold px-2 py-0.5 rounded-full bg-[var(--color-accent)]/10 text-[var(--color-accent)] border border-[var(--color-accent)]/20">{translate(t, design.difficultyKey)}</span>
                    <Icon className="w-4 h-4 text-[var(--color-accent)]" />
                  </div>
                  <h3 className="font-bold mb-1 group-hover:text-[var(--color-accent)] transition-colors">
                    {translate(t, design.nameKey)}
                  </h3>
                  <p className="text-sm text-[var(--color-text-secondary)] mb-2">{translate(t, design.descriptionKey)}</p>
                  <span className="text-xs text-[var(--color-text-muted)]">{translate(t, design.effectivenessKey)}</span>
                </Link>
              );
            })}
          </div>
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        ...
        <div className="max-w-3xl mx-auto space-y-3">
          {[
            'how_to_start', 'best_pets', 'base_protection', 'offline_income', 
            'raiding_tips', 'codes_working', 'private_server', 'rebirth_system'
          ].map((key) => (
      ...
      <section className="relative overflow-hidden py-16 full-width-bg">
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10 text-center">
          ...
          <div className="flex flex-wrap justify-center gap-4">
            <Link href={resolveDynamic(HOME_CONFIG.finalCta.guideHref, dynCtx)} className="btn-primary">
              {t(HOME_CONFIG.finalCta.guideLabelKey)}
              <ArrowRight className="w-4 h-4 ml-1" />
            </Link>
            <a href={EXTERNAL_LINKS[HOME_CONFIG.finalCta.externalLinkKey as keyof typeof EXTERNAL_LINKS]} target="_blank" rel="noopener noreferrer" className="btn-secondary">{t(HOME_CONFIG.finalCta.externalLabelKey)}</a>
          </div>
        </div>
      </section>
