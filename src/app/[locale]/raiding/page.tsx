...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }): Promise<Metadata> {
  ...
  return {
    ...
    alternates: {
      canonical: `${SITE_URL}/raiding`,
      languages: {
        'en': `${SITE_URL}/raiding`,
        'ko': `${SITE_URL}/ko/raiding`,
        'ja': `${SITE_URL}/ja/raiding`,
        'de': `${SITE_URL}/de/raiding`,
        'x-default': `${SITE_URL}/raiding`,
      },
    },
  };
}
...
export default async function Page({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const allContent = await getAllContent('raiding', validLocale);
  const articles = allContent.map(item => ({ slug: item.slug, metadata: item.metadata }));

  return <CategoryPage catKey="raiding" articles={articles} />;
}